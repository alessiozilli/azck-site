/* ========================================
   ...
   Phase 1 update: Added Booking, Gift Certificates to nav.
   Added Admin link in footer. Nav order per master plan.
   ======================================== */
(function () {
  'use strict';
  ...
  var isRoot =
    ...
    (!path.includes('/pages/') && path.split('/').length <= 2);
  ...
  for (var i = 0; i < navLinks.length; i++) {
    ...
    if (pageName === currentPage ||
        ...
        (currentPage === 'index' && pageName === 'home')) {
    ...
    // Special case: booking page without services hash
    if (currentPage === 'booking' && pageName === 'booking' && window.location.hash !== '#services') {
      link.classList.add('azck-nav__link--active');
    }
    // Preview pages highlight their parent category
